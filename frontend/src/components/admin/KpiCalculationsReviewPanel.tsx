...
import type { CompanyKpiAuditResult, KpiAuditRow } from '../../types/kpiAudit';

const MONTHS = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];

function KpiAuditTable({ title, rows, headerClass = 'bg-slate-50' }: {
  title: string;
  rows: KpiAuditRow[];
  headerClass?: string;
}) {
  ...
  return (
    <div className="bg-white border rounded-xl overflow-hidden">
      <div className={`px-4 py-3 border-b font-semibold text-base ${headerClass}`}>{title}</div>
...
interface AuditPayload {
  ...
  portfolio_loan_rows?: CompanyKpiAuditResult['rows'];
...
export function KpiCalculationsReviewPanel({ embedded }: Props) {
  ...
  return (
    <div className={`space-y-6 ${embedded ? '' : 'max-w-7xl mx-auto'}`}>
      ...
      {companyId === 'all' && payload?.portfolio_ops_rows && (
        <KpiAuditTable
          title="Portfolio — Rental Overview & AR Dashboard"
          rows={payload.portfolio_ops_rows}
        />
      )}

      {companyId === 'all' && payload?.portfolio_loan_rows && (
        <KpiAuditTable
          title="Portfolio — Loan Tracker"
          rows={payload.portfolio_loan_rows}
