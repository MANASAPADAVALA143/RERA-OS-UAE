...
const INSIGHTS: Insight[] = [
  ...
  { id:2,  priority:'HIGH',     category:'Partner Relations',  title:'Zero Distributions ù 100% Capital at Risk', text:'$2,223,677 of partner capital is fully deployed with 0% returned. No distributions have been made to any of the 5 partners. Pre-sale phase only.',       action:'Prepare distribution waterfall memo for partners. Trigger upon first lot sale.',                     quad:'UH' },
      ...
      <div className="grid grid-cols-5 gap-4">
        ...
        <div className="col-span-2 space-y-4">
          ...
          <div className="bg-white rounded-lg p-4 shadow-sm border border-red-100">
            ...
            {[
              'Summit Land: Cash covers only 1.1 months of EMI ù Aug shortfall $18,500',
              'No distributions made ù $2.50M partner capital fully at risk',
              '4 of 6 years net loss ù only 2024 profitable ($82,000)',
            ].map((a,i) => (
      ...
      <div className="grid grid-cols-2 gap-5">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-100">
            ...
            <p className="text-xs text-gray-400 mt-3">{checked.filter(Boolean).length}/{CHECKLIST_ITEMS.length} items complete ù saved automatically</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
        <div className="bg-gray-900 text-white px-4 py-2 text-sm font-bold">
          Summit Land Partner Capital Summary ù {partners.length} Partners
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-lg p-4 shadow-sm border border-amber-200">
          <p className="text-sm font-semibold text-gray-700 mb-1">Distribution Waterfall ù Pre-Sale</p>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
        <div className="bg-blue-700 text-white px-4 py-2 text-sm font-bold">
          Summit Land ù Partner Loan Exposure (Pro-Rata Share)
        </div>
        <div className="text-xs text-gray-500 px-4 py-2 bg-blue-50 border-b border-blue-100">
          Based on: Loan outstanding $1,800,000 ù Monthly EMI $18,500 ù Total cost basis $4,200,000
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
      ...
      <div className="space-y-2">
        {[
          { title:'No Distributions Made ù $2.50M Fully at Risk', color:'bg-amber-50 border-amber-200', textColor:'text-amber-800',
          ...
          { title:'Horizon Capital & Creekstone Lead at 45% Combined', color:'bg-blue-50 border-blue-200', textColor:'text-blue-800',
            text:'Top 2 partners hold 45% of equity. Remaining 3 partners average 18.3% each. Concentration is within acceptable range ù no diversification action needed for Phase 1.' },
        ].map((ins, i) => (
...
function pdKpis(fin: PDFinancials, y: number) {
  ...
  // QuickBooks exports income as negative credits in some formats ù take abs to keep revenue positive
            ...
            <tr key={i} className={`border-t border-gray-100 ${bg(item)}`}>
              ...
              {yrs.map(y=>(
                <td key={y} className={`py-1.5 px-3 text-right font-mono ${item.isNetIncome?'text-white':item.values[y]<0?'text-red-600':''}`}>
                  {item.values[y]===0?'ù':pdFmtFull(item.values[y])}
                </td>
              ))}
            </tr>
          ))}
...
function PDKPIView({ fin }: { fin: PDFinancials }) {
  ...
  if (ltv > 0 && ltv > 80) alerts.push({ type:'warning', text:`LTV at ${ltv.toFixed(1)}% ù above 80% threshold. Accelerated principal payment needed to unlock better refinance rates.` });
  if (iCov > 0 && iCov < 1.5) alerts.push({ type:'warning', text:`Interest coverage ${iCov.toFixed(2)}x ù below 1.5x safe harbor. NOI needs to grow from ${fmtShort(k.noi)} to ${fmtShort(k.interest * 1.5)}.` });
  if (consecutiveLossYears > 0) alerts.push({ type:'info', text:`Net loss for ${consecutiveLossYears} consecutive year(s) ù driven by interest (${fmtShort(k.interest)}) and other costs. NOI is ${k.noi >= 0 ? 'healthy' : 'stressed'} at ${fmtShort(k.noi)}.` });

  const uploadDate    = fin.uploadedAt ? new Date(fin.uploadedAt).toLocaleDateString('en-US', { month:'short', day:'numeric', year:'numeric' }) : 'ù';
  const uploadedFiles = [fin.plFile, fin.bsFile].filter(Boolean).join(' ù ') || 'No files uploaded';
  ...
  const tilesData = [
    ...
    { icon:<Building2 size={15}/>, label:'NOI ù Operating', value:fmtShort(k.noi),
  ...
  return (
    <div className="space-y-3">
      ...
      <div style={{background:'#1a2332',borderRadius:'10px',padding:'14px 18px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
        <div>
          <div style={{color:'#fff',fontSize:'15px',fontWeight:500}}>{fin.companyName} ù Financial Intelligence</div>
          <div style={{color:'#8899aa',fontSize:'11px',marginTop:'2px'}}>{uploadedFiles} ù {fin.years.join(' ù ')} ù Uploaded {uploadDate}</div>
        </div>
        ...
        <div className="col-span-3 bg-white rounded-xl p-4 border border-gray-100">
          <div className="flex items-start justify-between mb-3">
            <div>
              ...
              <div className="text-[10px] text-gray-400 mt-0.5">Revenue ù NOI ù Expenses ù Net Income</div>
            </div>
      ...
      {/* 4 ù BOT ROW: EXPENSE BAR + SCORECARD */}
      <div className="grid grid-cols-5 gap-3">
        <div className="col-span-2 bg-white rounded-xl p-4 border border-gray-100">
      ...
      <div className="grid grid-cols-4 gap-2">
        {([
          { icon:<Landmark size={15}/>, label:'Interest burden',     value:fmtShort(k.interest),         pct:Math.min(interestPct,100),         barColor:'bg-red-500',   bg:'bg-red-50',   border:'border-red-100',   of:'of revenue', note:`${interestPct.toFixed(1)}% of revenue ù largest single expense.` },
          { icon:<Settings size={15}/>, label:'Property tax load',   value:fmtShort(lastBreak.propTax),  pct:Math.min(propTaxPct,100),          barColor:'bg-amber-400', bg:'bg-amber-50', border:'border-amber-100', of:'of revenue', note:`${propTaxPct.toFixed(1)}% of revenue ù monitor for assessment increases.` },
          { icon:<Settings size={15}/>, label:'Management cost',     value:fmtShort(lastBreak.mgmt),     pct:Math.min(mgmtPct*10,100),          barColor:'bg-blue-500',  bg:'bg-blue-50',  border:'border-blue-100',  of:'of revenue', note:`${mgmtPct.toFixed(1)}% of revenue ù market 8ù10% ${mgmtPct<=10?'?':'ù above range'}.` },
          { icon:<TrendingUp size={15}/>, label:'Revenue growth YoY', value:revG!==null?`${revG>=0?'+':''}${revG.toFixed(1)}%`:'N/A', pct:Math.min(Math.abs(revG??0)*2,100), barColor:(revG??0)>=0?'bg-green-500':'bg-red-500', bg:(revG??0)>=0?'bg-green-50':'bg-red-50', border:(revG??0)>=0?'border-green-100':'border-red-100', of:`${lastY} vs ${prevY??'ù'}`, note:`Year-over-year revenue change.` },
        ] as const).map((card,i)=>(
      ...
      {/* 6 ù CFO ALERT BANNER */}
      {alerts.length>0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-3">
          <div className="flex items-center gap-2 mb-2">
            <AlertTriangle className="w-3.5 h-3.5 text-amber-600 flex-shrink-0"/>
            <span className="text-xs font-medium text-amber-800">CFO action items ù {fin.companyName}</span>
          </div>
...
function PDCFOView({ fin }: { fin: PDFinancials }) {
  ...
  if (k.interest>0) insights.push({color:'bg-blue-50 border-blue-200',text:`?? Interest expense is ${k.rev>0?(k.interest/k.rev*100).toFixed(1):0}% of revenue ù ${pdFmt(k.interest)}. Outstanding loans: ${pdFmt(k.loans)}.`});
  ...
  if (k.buildings>0) insights.push({color:'bg-gray-50 border-gray-200',text:`?? Buildings: ${pdFmt(k.buildings)} | Loans: ${pdFmt(k.loans)} | LTV: ${ltv.toFixed(1)}% ù ${ltv<80?'? Good (<80%)':ltv<90?'?? Watch (80ù90%)':'?? High (>90%)'}`});
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
        <div className="bg-gray-900 text-white px-4 py-2 text-sm font-bold">5-Year Snapshot ù {fin.companyName}</div>
        <table className="w-full text-xs">
...
export default function PropDevFinancials() {
  ...
  // All real PropDev companies from DB ù put them first in the dropdown
  ...
  const UploadBar = () => (
    <div className="flex flex-wrap items-center gap-2 p-3 bg-gray-50 border border-gray-200 rounded-lg">
      ...
      <span className="text-xs text-gray-500 flex-1">
        {uploadedFin
          ? <>P&L: <span className="font-medium text-gray-700">{uploadedFin.plFile||'ù'}</span> &nbsp;|&nbsp; B/S: <span className="font-medium text-gray-700">{uploadedFin.bsFile||'ù'}</span></>
      ...
      <button disabled={uploading} onClick={()=>plRef.current?.click()}
        ...
        <Upload size={12} />{uploading?'Uploadingù':'Upload P&L'}
      </button>
      <button disabled={uploading} onClick={()=>bsRef.current?.click()}
        ...
        <Upload size={12} />{uploading?'Uploadingù':'Upload B/S'}
      </button>
      {uploadedFin && (
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-xs text-gray-500">
            {uploadedFin
              ? `${uploadedFin.companyName} ù ${uploadedFin.years.length > 0 ? uploadedFin.years.join(', ') : 'no year data'}`
