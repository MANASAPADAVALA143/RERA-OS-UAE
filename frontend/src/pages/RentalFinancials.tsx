  ...
  return (
    <div className="flex flex-col items-center justify-center py-24 text-center">
      <div className="w-16 h-16 rounded-full flex items-center justify-center mb-4" style={{ background: '#F8FAFC' }}>
        <FileSpreadsheet className="w-8 h-8" style={{ color: '#5B5FEF' }} />
      </div>
      <h3 className="text-lg font-semibold mb-2" style={{ color: '#1A1D29' }}>No Financial Data Uploaded</h3>
...
function FinTable({ items, years, labelCol = 'Line Item', selectedYear, periods, periodKeys }: {
  ...
  const labelStyle = (item: FinItem, bg: string): React.CSSProperties => ({
    ...
    color: item.isSectionHeader ? '#92400E' : '#1A1D29',
    ...
    <div className="space-y-6">
      ...
      {!selectedYear && !period && netCFByYear.some(d => d.value !== 0) && (
        <div className="rounded-lg p-4 shadow-sm border" style={{ background: '#F8FAFC', borderColor: '#CBD5E1' }}>
          <p className="text-sm font-semibold mb-3" style={{ color: '#1A1D29' }}>Net Cash Flow by Year</p>
        ...
        {showBreakdown && auditRow && (
          <button
            ...
            style={{
              ...
              height: 26,
              borderRadius: '50%',
              border: `1px solid ${accent ? 'rgba(255,255,255,0.5)' : '#E8E9ED'}`,
            ...
            <div style={{ height: 28, marginTop: 8 }}>
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={trendData.map((v, i) => ({ x: i, y: v }))}>
                  <Line type="monotone" dataKey="y" stroke={accent ? 'rgba(255,255,255,0.7)' : '#5B5FEF'} dot={false} strokeWidth={1.5} isAnimationActive={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          )}
...
function KPITab({
  ...
  return (
    <div className="space-y-6">
      <p style={{ fontSize: 13, color: '#A8A29E' }}>KPIs for selected period: <strong style={{ color: '#1A1D29' }}>{label}</strong>{compareLabel ? <span> · compared to <strong style={{ color: '#1A1D29' }}>{compareLabel}</strong></span> : null}</p>
      ...
      <div style={{ background: '#FFFFFF', border: '1px solid #E8E9ED', borderRadius: 12, padding: 20 }}>
        <p style={{ fontSize: 15, fontWeight: 600, color: '#1A1D29', marginBottom: 4 }}>Margins &amp; Ratios Trend</p>
        ...
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={marginsTrend} margin={{ left: 0, right: 16, top: 4, bottom: 4 }}>
            ...
            <Tooltip formatter={(v: number) => `${v.toFixed(1)}%`} contentStyle={{ background: '#FFFFFF', border: '1px solid #E8E9ED', borderRadius: 8, fontSize: 13 }} />
            <Legend iconSize={10} wrapperStyle={{ fontSize: 12 }} />
            <Line type="monotone" dataKey="NOI Margin %"    stroke="#5B5FEF" strokeWidth={2} dot={{ r: 3, fill: '#5B5FEF' }} />
      ...
      <div style={{ background: '#FFFFFF', border: '1px solid #E8E9ED', borderRadius: 12, padding: 20 }}>
        <p style={{ fontSize: 15, fontWeight: 600, color: '#1A1D29', marginBottom: 4 }}>5-Year Financial Trend</p>
        ...
        <ResponsiveContainer width="100%" height={240}>
          <LineChart data={trendData} margin={{ left: 16, right: 16, top: 4, bottom: 4 }}>
            ...
            <XAxis dataKey="year" tick={{ fontSize: 11 }} />
            <YAxis tickFormatter={v => fmt(v as number)} tick={{ fontSize: 10 }} />
            <Tooltip formatter={(v: number) => fmtFull(v)} contentStyle={{ background: '#FFFFFF', border: '1px solid #E8E9ED', borderRadius: 8, fontSize: 13 }} />
            <Legend iconSize={10} wrapperStyle={{ fontSize: 12 }} />
            <Line type="monotone" dataKey="Revenue"    stroke="#5B5FEF" strokeWidth={2} dot={{ r: 3 }} />
      ...
      <div className="grid grid-cols-2 gap-4">
        <div style={{ background: '#FFFFFF', border: '1px solid #E8E9ED', borderRadius: 12, padding: 20 }}>
          <p style={{ fontSize: 15, fontWeight: 600, color: '#1A1D29', marginBottom: 14 }}>Revenue Allocation ({label})</p>
          <ResponsiveContainer width="100%" height={220}>
            <PieChart>
        ...
        {yoyComparison.length > 0 && (
          <div style={{ background: '#FFFFFF', border: '1px solid #E8E9ED', borderRadius: 12, padding: 20 }}>
            <p style={{ fontSize: 15, fontWeight: 600, color: '#1A1D29', marginBottom: 14 }}>{label} vs {compareLabel || 'Prior Period'}</p>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={yoyComparison}>
                ...
                <Tooltip formatter={(v: number) => v.toFixed(2)} contentStyle={{ background: '#FFFFFF', border: '1px solid #E8E9ED', borderRadius: 8, fontSize: 13 }} />
                <Legend iconSize={10} wrapperStyle={{ fontSize: 12 }} />
                <Bar dataKey="current"  name={label}  fill="#5B5FEF" radius={[4,4,0,0]} />
...
const CFO_TT = {
  contentStyle: { background: '#FFFFFF', border: '1px solid #E8E9ED', borderRadius: 8, fontSize: 13, color: '#1A1D29' },
  labelStyle: { color: '#57534E', fontWeight: 600, fontSize: 13 },
  itemStyle: { color: '#1A1D29', fontSize: 13 },
};
    ...
    <div style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 8, padding: '14px 16px', marginTop: 12 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 10 }}>
        ...
        <button
          type="button"
          onClick={onClear}
          style={{ fontSize: 12, color: '#5B5FEF', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600, textDecoration: 'underline', padding: 0 }}
        >
          × clear drill-down
        </button>
      </div>
            ...
            <tbody>
              {rows.map((r, i) => (
                <tr key={i} style={{ borderBottom: '1px solid rgba(253,230,138,0.4)', background: i % 2 === 0 ? '#FFFBEB' : '#FEF9E7' }}>
                  ...
                  <td style={{ padding: '8px 10px', textAlign: 'right', fontWeight: 600, color: '#1A1D29', fontFamily: 'monospace' }}>{fmtFull(r.amount)}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
...
function CFOTab({
  ...
  if (margin > 20) {
    ...
    InsightIcon = <TrendingUp size={20} style={{ color: '#5B5FEF', flexShrink: 0 }} />;
  ...
  const OPEX_PALETTE = ['#5B5FEF','#F2994A','#2F80ED','#166534','#B91C1C','#9B59B6','#818CF8','#E8E9ED'];
  ...
  return (
    <div className="space-y-6">
      <style>{`
        ...
        .cfo-cat-row:hover { background: #F7F8FA; border-radius: 6px; cursor: pointer; }
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            ...
            <div style={{ background: '#FFFFFF', border: '0.5px solid #E8E9ED', borderRadius: 8, padding: 16 }}>
              <p style={{ fontSize: 15, fontWeight: 600, color: '#1A1D29', marginBottom: 4 }}>Revenue Mix</p>
              ...
              {periodAgg.totalRevenue > 0 ? (
                <>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginTop: 8 }}>
                    {[
                      { name: 'Rent Income',  val: periodAgg.rentIncome,  color: '#5B5FEF' },
                      ...
                      >
                        ...
                        <div style={{ textAlign: 'right' }}>
                          <span style={{ fontSize: 13, fontWeight: 600, color: '#1A1D29', fontFamily: 'monospace' }}>{fmtFull(s.val)}</span>
                    ...
                    <div style={{ borderTop: '1px solid #E8E9ED', paddingTop: 6, display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ fontSize: 13, fontWeight: 600, color: '#78716C' }}>Total Revenue</span>
                      <span style={{ fontSize: 14, fontWeight: 700, color: '#1A1D29', fontFamily: 'monospace' }}>{fmtFull(periodAgg.totalRevenue)}</span>
                    </div>
                  </div>
                </>
              ) : (
            ...
            <div style={{ background: '#FFFFFF', border: '0.5px solid #E8E9ED', borderRadius: 8, padding: 16 }}>
              <p style={{ fontSize: 15, fontWeight: 600, color: '#1A1D29', marginBottom: 4 }}>Opex Breakdown</p>
              <p className="cfo-chart-hint">Click a bar to drill into P&amp;L expense lines</p>
              {drillOpexCat && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                  <span style={{ fontSize: 12, fontWeight: 600, padding: '2px 10px', borderRadius: 20, background: '#5B5FEF', color: '#fff' }}>{drillOpexCat}</span>
                      ...
                      {cats.map((c, i) => (
                        ...
                        >
                          ...
                          <div style={{ display: 'flex', gap: 10, fontFamily: 'monospace' }}>
                            <span style={{ color: '#1A1D29', fontWeight: 600 }}>{fmt(c.val)}</span>
            ...
            <div style={{ background: '#FFFFFF', border: '0.5px solid #E8E9ED', borderRadius: 8, padding: 16 }}>
              <p style={{ fontSize: 15, fontWeight: 600, color: '#1A1D29', marginBottom: 12 }}>Profitability Trend</p>
            ...
            <div style={{ background: '#FFFFFF', border: '0.5px solid #E8E9ED', borderRadius: 8, padding: 16 }}>
              <p style={{ fontSize: 15, fontWeight: 600, color: '#1A1D29', marginBottom: 4 }}>Revenue by Month</p>
                ...
                <>
                  <ResponsiveContainer width="100%" height={200}>
                    ...
                    >
                      ...
                      <Legend iconSize={8} wrapperStyle={{ fontSize: 12, color: '#57534E' }} />
                      <Bar dataKey="rentIncome"  stackId="rev" fill="#5B5FEF" name="Rent Income"  radius={[0,0,0,0]} className="cfo-bar-clickable" />
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', marginTop: '12px' }}>
              {[
                ...
                <div key={item.label}>
                  ...
                  <p style={{ fontSize: 18, fontWeight: 700, color: '#1A1D29', marginTop: 2 }}>{item.value}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {snapshotRows.map((r, i) => (
                <tr key={i} style={{ background: r.year === selectedYear ? '#EEF0FF' : i % 2 === 0 ? '#F7F8FA' : '#FFFFFF', borderTop:'1px solid #E8E9ED' }}>
                  <td style={{ padding:'8px 16px', fontSize:14, fontWeight: r.year === selectedYear ? 700 : 500, color:'#1A1D29' }}>{r.year}{r.year === selectedYear ? ' ◀' : ''}</td>
                  <td style={{ padding:'8px 16px', textAlign:'right', fontFamily:'monospace', fontSize:14, color:'#1A1D29' }}>{fmt(r.revenue)}</td>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div style={{ background: '#FFFFFF', border: '0.5px solid #E8E9ED', borderRadius: 8, padding: 16 }}>
          ...
          <p style={{ fontSize: 20, fontWeight: 700, color: latestRow.netIncome >= 0 ? '#1A1D29' : '#B91C1C', marginTop: 8 }}>{latestRow.netIncome < 0 ? `(${fmtFull(Math.abs(latestRow.netIncome))})` : fmtFull(latestRow.netIncome)}</p>
        ...
        <div style={{ background: '#FFFFFF', border: '0.5px solid #E8E9ED', borderRadius: 8, padding: 16 }}>
          ...
          <p style={{ fontSize: 20, fontWeight: 700, color: avgMargin >= 0 ? '#1A1D29' : '#B91C1C', marginTop: 8 }}>{avgMargin.toFixed(1)}%</p>
      ...
      <div className="space-y-3">
        ...
        {insights.length === 0
          ...
          : insights.map((ins, i) => (
              <div key={i} style={{ border:'1px solid #E8E9ED', borderRadius:8, padding:16, background:'#FFFFFF' }}>
...
const FM_INPUT: React.CSSProperties = {
  width: '100%', padding: '8px 12px', border: '1px solid #E8E9ED',
  borderRadius: 8, fontSize: 14, color: '#1A1D29', background: '#FFFFFF',
...
function FinancialMetricsTab({ companyName }: { companyName: string }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 style={{ fontSize: 22, fontWeight: 700, color: '#1A1D29', marginBottom: 6 }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 680 }}>
        {/* Month */}
        <div style={{ background: '#FFFFFF', border: '1px solid #E8E9ED', borderRadius: 12, padding: '16px 18px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, paddingTop: 4 }}>
          <button
            ...
            style={{
              ...
              background: 'linear-gradient(135deg,#5B5FEF,#7C3AED)', color: '#fff',
...
export default function RentalFinancials() {
  ...
  const selectStyle: React.CSSProperties = {
    fontSize: 13, border: '1px solid #E8E9ED', borderRadius: 6,
    padding: '5px 10px', background: '#FFFFFF', color: '#1A1D29', cursor: 'pointer',
  };
      ...
      {isAll ? (
        <div className="border rounded-2xl shadow-sm p-6" style={{ background: '#F8FAFC', borderColor: '#CBD5E1' }}>
          ...
          {loadingFin && Object.keys(allFinancials).length === 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '48px 0', gap: 12 }}>
              <div style={{ width: 32, height: 32, border: '3px solid #E8E9ED', borderTopColor: '#5B5FEF', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
        ...
        <div className="space-y-4">
          ...
          <div className="border rounded-2xl shadow-sm p-4 flex items-center justify-between gap-4 flex-wrap" style={{ background: '#F8FAFC', borderColor: '#CBD5E1' }}>
            <div>
              <h1 style={{ fontSize: 22, fontWeight: 700, color: '#1A1D29' }}>{currentFin.companyName}</h1>
            ...
            <div className="flex items-center gap-1.5 flex-wrap" style={{ justifyContent: 'flex-end' }}>
              {activeTab === 'CFO Dashboard' && currentFin ? (
                <div style={{
                  ...
                  background: '#FFFFFF', border: '0.5px solid #E8E9ED', borderRadius: 8, padding: '10px 12px',
                }}>
                ...
                <>
                  ...
                  <span style={{
                    fontSize: 11, color: '#78716C', background: '#F7F8FA',
                    border: '1px solid #E8E9ED', borderRadius: 20, padding: '3px 12px',
                  }}>
              ...
              {selectedYear ? (
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 700, color: '#5B5FEF', padding: '3px 10px', borderRadius: 20, background: '#FDF3D7', border: '1.5px solid #5B5FEF' }}>
                ...
                currentFin.years.map(y => (
                  <button
                    ...
                    style={{
                      ...
                      border: selectedYear === y ? '1.5px solid #5B5FEF' : '1.5px solid #C8C0B0',
                      background: selectedYear === y ? '#FDF3D7' : 'transparent',
                      color: selectedYear === y ? '#5B5FEF' : '#78716C',
          ...
          <div className="flex gap-1 p-1 rounded-lg w-fit flex-wrap" style={{ background: '#E8E4DC' }}>
            {visibleTabs.map(t => (
              <button key={t} onClick={() => {
                ...
                style={activeTab === t
                  ? { background: '#5B5FEF', color: '#1E1B4B', boxShadow: '0 1px 3px rgba(0,0,0,0.15)' }
