...
class CostTrade(Base):
    ...
    sov_type: Mapped[str | None] = mapped_column(String(50), nullable=True)
    sov_status: Mapped[str | None] = mapped_column(String(50), nullable=True)
    sov_start_date: Mapped[date | None] = mapped_column(Date, nullable=True)
    sov_end_date: Mapped[date | None] = mapped_column(Date, nullable=True)
    prior_period_completed: Mapped[float | None] = mapped_column(Numeric(14, 2), nullable=True)
    current_period_completed: Mapped[float | None] = mapped_column(Numeric(14, 2), nullable=True)
    stored_materials: Mapped[float] = mapped_column(Numeric(14, 2), default=0, nullable=False, server_default="0")
    retainage_pct: Mapped[float | None] = mapped_column(Numeric(5, 4), nullable=True)
