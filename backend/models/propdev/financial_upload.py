...
class PropDevFinancialUpload(Base):
    """Parsed P&L / Balance Sheet / Cash Flow per Property Dev company (mirrors r_financial_uploads)."""
    __tablename__ = "propdev_financial_uploads"

    id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), primary_key=True, default=uuid.uuid4)
    tenant_id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), nullable=False, index=True)
    company_id: Mapped[uuid.UUID] = mapped_column(
        Uuid(as_uuid=True), ForeignKey("propdev_companies.id", ondelete="CASCADE"), nullable=False, index=True,
    )
    ...
    __table_args__ = (Index("ix_propdev_fin_uploads_tenant_company", "tenant_id", "company_id"),)
